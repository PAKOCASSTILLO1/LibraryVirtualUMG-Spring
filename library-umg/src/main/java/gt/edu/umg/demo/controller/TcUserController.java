...
public class TcUserController {
    ...
    @GetMapping("/{userId}")
    public ApiResponse findById(@PathVariable(value = "userId") Integer userId) {
        try {
    ...
    @PutMapping("/{userId}")
    public ApiResponse updateUser(@PathVariable(value = "userId") Integer userId, @Valid @RequestBody TcUser tcUser) {
        try {
            Optional<TcUser> found = tcUserRepository.findById(userId);
