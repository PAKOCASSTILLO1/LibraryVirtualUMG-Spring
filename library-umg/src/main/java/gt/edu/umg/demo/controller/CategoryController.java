...
public class CategoryController {
    ...
    public ResponseEntity<ApiResponse> ver(@PathVariable int id){
        try {
            List<Category> data = new ArrayList<>();
            data.add(categoryRepository.findById(id).orElse(null));
            ApiResponse apiResponse = new ApiResponse("OK", "Proceso Exitoso", data);
            return new ResponseEntity<ApiResponse>(apiResponse, HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<ApiResponse> ver(@RequestBody Category category){
        try {
            List<Category> data = new ArrayList<>();
            data.add(categoryRepository.save(category));
            ApiResponse apiResponse = new ApiResponse("OK", "Proceso Exitoso", data);
            return new ResponseEntity<ApiResponse>(apiResponse, HttpStatus.OK);
        } catch (Exception e) {
            ApiResponse apiResponse = new ApiResponse("FAIL", e.getMessage(), null);
    ...
    public ResponseEntity<ApiResponse> editar(@PathVariable int id, @RequestBody Category body){
        try {
            List<Category> data = new ArrayList<>();
            Category category = categoryRepository.findById(id).orElse(null);            
            data.add(categoryRepository.save(category.modified(body)));
            ApiResponse apiResponse = new ApiResponse("OK", "Proceso Exitoso", data);
            return new ResponseEntity<ApiResponse>(apiResponse, HttpStatus.OK);
        } catch (Exception e) {
            if (e.toString().equals("java.lang.NullPointerException")) {
