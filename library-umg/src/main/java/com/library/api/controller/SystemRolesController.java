...
import com.library.api.repository.SystemRolesRepository;
...
public class SystemRolesController {
    ...
    @Autowired
    SystemRolesRepository systemRolesRepository;

    @GetMapping("/listar")
    public ResponseEntity<ApiResponse> listar(){
        List<SystemRoles> data = new ArrayList<>();
        List<SystemRoles> systemRoless =  systemRolesRepository.findAll();
        for (SystemRoles systemRoles : systemRoless) {
            if (systemRoles.getState()!=0) {
                data.add(systemRoles);                    
            }
        }
        try {
    ...
    public ResponseEntity<ApiResponse> ver(@PathVariable int id){
        try {
            List<SystemRoles> data = new ArrayList<>();
            data.add(systemRolesRepository.findById(id).orElse(null));
            ApiResponse apiResponse = new ApiResponse("OK", "Proceso Exitoso", data);
            return new ResponseEntity<ApiResponse>(apiResponse, HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<ApiResponse> ver(@RequestBody SystemRoles systemRoles){
        try {
            List<SystemRoles> data = new ArrayList<>();
            data.add(systemRolesRepository.save(systemRoles));
            ApiResponse apiResponse = new ApiResponse("OK", "Proceso Exitoso", data);
            return new ResponseEntity<ApiResponse>(apiResponse, HttpStatus.OK);
        } catch (Exception e) {
            ApiResponse apiResponse = new ApiResponse("FAIL", e.getMessage(), null);
    ...
    public ResponseEntity<ApiResponse> editar(@PathVariable int id, @RequestBody SystemRoles body){
        try {
            List<SystemRoles> data = new ArrayList<>();
            SystemRoles systemRoles = systemRolesRepository.findById(id).orElse(null);            
            data.add(systemRolesRepository.save(systemRoles.modified(body)));
            ApiResponse apiResponse = new ApiResponse("OK", "Proceso Exitoso", data);
            return new ResponseEntity<ApiResponse>(apiResponse, HttpStatus.OK);
        } catch (Exception e) {
            if (e.toString().equals("java.lang.NullPointerException")) {
